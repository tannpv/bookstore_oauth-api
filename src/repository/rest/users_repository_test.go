...
func TestLoginUserInvalidUserJsonResponse(t *testing.T) {
	...
	rest.AddMockups(&rest.Mock{
		...
		RespBody:     `{"id":"1", "first_name":"tan","last_name":"nguyen","email":"tannpv@gmail.com"}`,
	})
	repository := usersRepository{}
	user, err := repository.LoginUser("email@gmail.com", "the-password")
	assert.Nil(t, user)
...
func TestLoginUserNoError(t *testing.T) {
	...
	assert.EqualValues(t, "tan", user.FirstName)
	assert.EqualValues(t, "nguyen", user.LastName)
	assert.EqualValues(t, "tannpv@gmail.com", user.Email)
}
